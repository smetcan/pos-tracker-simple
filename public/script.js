...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function navigateTo(page) {
        ...
        try {
            switch (page) {
                ...
                case 'bulgular': mainContent.innerHTML = getBulgularHTML(); navLinks.bulgular.classList.add('active'); break;
    ...
    function getBulgularHTML() { return '<h1 class="text-3xl font-bold">Bulgu Takibi</h1><p>Bu sayfa yakında yapılacak.</p>'; }
    ...
    function getYonetimHTML(vendors, models, versions, activeTab) {
        ...
        const modelsTableRows = models.map(model => `
             <tr class="border-b"><td class="p-3">${model.id}</td><td class="p-3 font-medium">${model.name}</td><td class="p-3">${model.code || ''}</td><td class="p-3">${model.vendorName}</td><td class="p-3 text-right">
        ...
        const versionsTableRows = versions.map(version => {
            ...
            return `
             <tr class="border-b">
                <td class="p-3 font-medium">${version.versionNumber}</td>
                <td class="p-3">${version.vendorName}</td>
                <td class="p-3">${version.deliveryDate}</td>
                <td class="p-3 text-xs text-gray-600">${version.models || ''}</td>
                <td class="p-3"><span class="px-2 py-1 text-xs font-medium rounded-full ${statusClass}">${version.status}</span></td>
                <td class="p-3">${version.prodOnayDate || '-'}</td>
                ...
                <div class="p-6">
                    ...
                    <div id="models-tab" class="tab-content ${activeTab === 'models' ? 'active' : ''}">
                        ...
                        <div class="rounded-md border"><table class="w-full text-sm"><thead><tr class="border-b">
                            ...
                            <th class="p-3 text-left sortable-header" data-table="models" data-sort-key="vendorName">Ait Olduğu Vendor ${getSortIcon(modelSort, 'vendorName')}</th>
                    ...
                    <div id="versions-tab" class="tab-content ${activeTab === 'versions' ? 'active' : ''}">
                        ...
                        <div class="rounded-md border"><table class="w-full text-sm"><thead><tr class="border-b">
                            <th class="p-3 text-left">Versiyon No</th>
                            <th class="p-3 text-left">Vendor</th>
                            <th class="p-3 text-left">Teslim Tarihi</th>
                            <th class="p-3 text-left">Geçerli Modeller</th>
                            <th class="p-3 text-left">Durum</th>
                            <th class="p-3 text-left">Prod Onay Tarihi</th>
                            <th class="p-3 text-right">İşlemler</th>
                        </tr></thead><tbody>${versionsTableRows}</tbody></table></div>
                    </div>
                </div>
            </div>`;
    }
    ...
    async function renderYonetimPage() {
        ...
        const sortedModels = sortData(modelsData, modelSort);
            ...
            button.addEventListener('click', () => {
                ...
                document.getElementById('confirm-delete').addEventListener('click', async () => {
                    try {
                        ...
                        vendorsData = []; modelsData = []; versionsData = [];
    ...
    function attachVersionModalListeners(version = null) {
        ...
        // Eğer düzenleme modundaysa, formu ilk açılışta doldur
        if (isEdit) {
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                const data = {
                    ...
                    prodOnayDate: formData.get('prodOnayDate'),
